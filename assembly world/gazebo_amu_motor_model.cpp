...
#include "gazebo_amu_motor_model.h"
...
void GazeboMotorModel::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf) {
  ...
  // Get robot name and joint
  if (_sdf->HasElement("robotNamespace"))
    namespace_ = _sdf->GetElement("robotNamespace")->Get<std::string>();
  else
    gzerr << "[gazebo_amu_motor_model] Please specify a robotNamespace.\n";
  ...
  if (_sdf->HasElement("jointName"))
    ...
    gzerr << "[gazebo_amu_motor_model] Please specify a jointName, where the rotor is attached.\n";
  
  // Get the pointer to the joint.
  joint_ = model_->GetJoint(joint_name_);
  if (joint_ == NULL)
    gzthrow("[gazebo_amu_motor_model] Couldn't find specified joint \"" << joint_name_ << "\".");

  // (Optional) setup joint control pid to control joint
  ...
  if (_sdf->HasElement("linkName"))
    ...
    gzerr << "[gazebo_amu_motor_model] Please specify a linkName of the rotor.\n";
  link_ = model_->GetLink(link_name_);
  if (link_ == NULL)
    gzthrow("[gazebo_amu_motor_model] Couldn't find specified link \"" << link_name_ << "\".");

  // Get the number (sequence) of rotor 0/1/2/3 
  if (_sdf->HasElement("motorNumber"))
    motor_number_ = _sdf->GetElement("motorNumber")->Get<int>();
  else
    gzerr << "[gazebo_amu_motor_model] Please specify a motorNumber.\n";

  // Get turning direction (cw or ccw)
  if (_sdf->HasElement("turningDirection")) {
    ...
    if (turning_direction == "cw")
      ...
      gzerr << "[gazebo_amu_motor_model] Please only use 'cw' or 'ccw' as turningDirection.\n";
  }
  else
    gzerr << "[gazebo_amu_motor_model] Please specify a turning direction ('cw' or 'ccw').\n";
  ...
  getSdfParam<double>(_sdf, "motorConstant2", motor_constant_2, motor_constant_2);
  getSdfParam<double>(_sdf, "motorConstant3", motor_constant_3, motor_constant_3);
  getSdfParam<double>(_sdf, "momentConstant", moment_constant_, moment_constant_);
  getSdfParam<double>(_sdf, "momentConstant2", moment_constant_2, moment_constant_2);
  getSdfParam<double>(_sdf, "momentConstant3", moment_constant_3, moment_constant_3);
  // Get first order filter params
  ...
  */
  ...
  //std::cout << "[gazebo_amu_motor_model]: Subscribe to gz topic: "<< motor_failure_sub_topic_ << std::endl;
...
void GazeboMotorModel::UpdateForcesAndMoments() {
  ...
  double force = real_motor_velocity * real_motor_velocity * motor_constant_ + real_motor_velocity * motor_constant_2 + motor_constant_3;
  ...
  ignition::math::Vector3d drag_torque(0, 0, -turning_direction_ * (real_motor_velocity * real_motor_velocity * moment_constant_ + moment_constant_2 * real_motor_velocity + moment_constant_3));
